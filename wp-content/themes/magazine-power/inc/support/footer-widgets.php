 ...
 * @since 1.0.0
 */
class Magazine_Power_Footer_Widgets {
	 ...
	 * @var int
	 */
	protected $theme_prefix = 'magazine_power';
		...
		for ( $i = 1; $i <= $this->max_widgets; $i++ ) {
			register_sidebar(
				array(
					...
					'name'          => sprintf( esc_html__( 'Footer Widget %1$s', 'magazine-power' ), $i ),
	...
	public function add_footer_widgets() {
		...
		echo $footer_widgets_content; // phpcs:ignore WordPress.Security.EscapeOutput
	}
	 ...
	 */
	protected function all_active_widgets() {
		$arr = array();
	 ...
	 */
	protected function get_footer_widgets_content( $args ) {
		...
		echo $container_open; // phpcs:ignore WordPress.Security.EscapeOutput
		...
		echo $container_close; // phpcs:ignore WordPress.Security.EscapeOutput
...
// Initialize.
$magazine_power_footer_widgets = new Magazine_Power_Footer_Widgets();
